...
@Mapper
public interface PpSicboResultMapper {
    // 修改为批量插入实现
    @Insert({
        "<script>",
        "INSERT INTO pp_sicbo_result(table_id, round_id, dice1, dice2, dice3) VALUES ",
        "<foreach collection='resultList' item='item' separator=','>",
        "(#{item.tableId}, #{item.roundId}, #{item.dice1}, #{item.dice2}, #{item.dice3})",
        "</foreach>",
        "</script>"
    })
    void insertList(List<PpSicboResult> resultList);

    @Select("SELECT * FROM pp_sicbo_result WHERE table_id = #{tableId} ORDER BY id DESC LIMIT #{limit}")
    List<PpSicboResult> findLatestByTableId(String tableId, int limit);
    ...
    @Select({"<script>",
        ...
        "FROM (",
        "   SELECT *, (dice1 + dice2 + dice3) AS total ,case when dice1=dice2 and dice1=dice3 then 1 else 0 end as is_triple FROM pp_sicbo_result",
    ...
    @Select("SELECT MAX(create_time) AS latest_time FROM pp_sicbo_result")
