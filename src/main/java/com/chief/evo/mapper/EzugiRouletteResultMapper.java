...
@Mapper
public interface EzugiRouletteResultMapper {
    @Insert({
        "<script>",
        "INSERT ignore INTO ezugi_roulette_result(table_id, round_id, result, color) VALUES ",
        "<foreach collection='resultList' item='item' separator=','>",
        "(#{item.tableId}, #{item.roundId}, #{item.result}, #{item.color})",
        "</foreach>",
        "</script>"
    })
    void insertList(List<EzugiRouletteResult> resultList);
    
    @Select("SELECT * FROM ezugi_roulette_result WHERE table_id = #{tableId} ORDER BY id DESC LIMIT #{limit}")
    List<EzugiRouletteResult> findLatestByTableId(String tableId, int limit);
    ...
    @Select("SELECT MAX(create_time) AS latest_time FROM ezugi_roulette_result")
