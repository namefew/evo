...
import com.chief.evo.entity.WlRouletteResult;
...
@Mapper
public interface WlRouletteResultMapper {
    @Select({
            "<script>",
            "SELECT",
    ...
    @Select("SELECT MAX(create_time) AS latest_time FROM wl_roulette_result")
    Optional<LocalDateTime> findLatestCreateTimestamp();

    @Insert({
            "<script>",
            "INSERT ignore INTO wl_roulette_result(table_id, result, round_id, dealer_id,round) VALUES ",
            "<foreach collection='resultList' item='item' separator=','>",
            "(#{item.tableId}, #{item.result},#{item.roundId},#{item.dealerId},#{item.round})",
            "</foreach>",
            "</script>"
    })
    void insertList(List<WlRouletteResult> resultList);
