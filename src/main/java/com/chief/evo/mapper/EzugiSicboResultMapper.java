...
@Mapper
public interface EzugiSicboResultMapper {
    @Insert({
        "<script>",
        "INSERT ignore INTO ezugi_sicbo_result(table_id, round_id, dice1, dice2, dice3) VALUES ",
        "<foreach collection='resultList' item='item' separator=','>",
        "(#{item.tableId}, #{item.roundId}, #{item.dice1}, #{item.dice2}, #{item.dice3})",
        "</foreach>",
        "</script>"
    })
    void insertList(List<EzugiSicboResult> resultList);

    @Select("SELECT * FROM ezugi_sicbo_result WHERE table_id = #{tableId} ORDER BY id DESC LIMIT #{limit}")
    List<EzugiSicboResult> findLatestByTableId(String tableId, int limit);
    ...
    @Select({"<script>",
            ...
            "FROM (",
            "   SELECT * ,dice1+dice2+dice3 as total ,case when dice1=dice2 and dice1=dice3 then 1 else 0 end as is_triple FROM ezugi_sicbo_result",
    ...
    @Select("SELECT MAX(create_time) AS latest_time FROM ezugi_sicbo_result")
