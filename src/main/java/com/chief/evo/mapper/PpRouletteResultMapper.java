...
@Mapper
public interface PpRouletteResultMapper {
    @Insert({
            "<script>",
            "INSERT INTO pp_roulette_result(table_id, round_id, result, color) VALUES ",
            "<foreach collection='resultList' item='item' separator=','>",
            "(#{item.tableId}, #{item.roundId}, #{item.result}, #{item.color})",
            "</foreach>",
            "</script>"
    })
    void insertList(List<PpRouletteResult> resultList);

    @Select("SELECT * FROM pp_roulette_result WHERE table_id = #{tableId} ORDER BY id DESC LIMIT #{limit}")
    List<PpRouletteResult> findLatestByTableId(String tableId, int limit);
    ...
    @Select("SELECT MAX(create_time) AS latest_time FROM pp_roulette_result")
