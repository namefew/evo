...
import com.chief.evo.entity.WlColorDiskResult;
...
@Mapper
public interface WlColorDiskResultMapper {
    @Insert({
        "<script>",
        "INSERT ignore INTO wl_color_disk_result(table_id, result, round_id, dealer_id,round) VALUES ",
        "<foreach collection='resultList' item='item' separator=','>",
        "(#{item.tableId}, #{item.result},#{item.roundId},#{item.dealerId},#{item.round})",
        "</foreach>",
        "</script>"
    })
    void insertList(List<WlColorDiskResult> resultList);
    
    @Select("SELECT * FROM wl_color_disk_result WHERE table_id = #{tableId} ORDER BY id DESC LIMIT #{limit}")
    List<WlColorDiskResult> findLatestByTableId(String tableId, int limit);
    ...
    @Select("SELECT MAX(create_time) AS latest_time FROM wl_color_disk_result")
