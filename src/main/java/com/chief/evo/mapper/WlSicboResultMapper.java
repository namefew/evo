...
import com.chief.evo.entity.WlSicboResult;
...
@Mapper
public interface WlSicboResultMapper {
    @Select({"<script>",
            "SELECT",
            "   table_id AS tableId,",
            ...
            "FROM (",
            "   SELECT * ,case when dice1=dice2 and dice1=dice3 then 1 else 0 end as is_triple FROM wl_sicbo_result",
    ...
    @Select("SELECT MAX(create_time) AS latest_time FROM wl_sicbo_result")
    Optional<LocalDateTime> findLatestCreateTimestamp();

    @Insert({
            "<script>",
            "INSERT ignore INTO wl_sicbo_result(table_id, round_id, dice1, dice2, dice3, dealer_id,round) VALUES ",
            "<foreach collection='resultList' item='item' separator=','>",
            "(#{item.tableId}, #{item.roundId}, #{item.dice1}, #{item.dice2}, #{item.dice3},#{item.dealerId},#{item.round})",
            "</foreach>",
            "</script>"
    })
    void insertList(List<WlSicboResult> resultList);
